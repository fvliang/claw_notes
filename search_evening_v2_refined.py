#!/usr/bin/env python3
"""LLM serving paper search - 2026-04-27 evening run v2 (refined queries, longer delays)"""
...
NOT_SERVING_KEYWORDS = [
    ...
    'retriever', 'dense retrieval', 'code verification',
    'math evaluation', 'judge framework',
]
...
def is_llm_serving(title, abstract):
    ...
    # Need at least 2 serving keywords OR 1 serving + no non-serving + LLM
    # And serving keywords must outnumber non-serving
    if ns >= 2:
        return False
...
# ── ArXiv search with broader queries ──
all_papers = []
seen_titles = set()

arxiv_queries = [
    "cat:cs.AI+cat:cs.DC AND (llm serving OR llm inference)",
    "speculative decoding large language model",
    "llm inference efficient low latency",
    "kv cache management large language model serving",
    "llm serving system throughput batching",
    ...
    "moe llm inference serving load balancing",
    "efficient llm inference quantization serving",
    "llm inference scheduling request routing",
    "llm serving memory management paged attention",
    "distributed llm inference serving parallel",
    "llm inference draft model speculative",
]

print("\n🔍 ArXiv Search (Evening v2)")
...
def search_s2(query, limit=10, year_from=2025):
...
s2_queries = [
    ...
    "KV cache LLM serving memory management",
    "LLM serving throughput scheduling batching",
    "LLM inference disaggregation prefill decode",
]

print("\n🔍 Semantic Scholar Search (with longer delays)")
for q in s2_queries:
    print(f"  Searching S2: {q}")
    results = search_s2(q, limit=10, year_from=2025)
    found = 0
    for p in results:
...
print(f"  New (not in DB): {len(new_papers)}")
...
for p in new_papers:
    ...
    if not os.path.exists(filepath):
        ...
        abstract_cn = "[中文翻译待补充] " + abstract_en[:200] + "..."
