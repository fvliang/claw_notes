#!/usr/bin/env python3
"""LLM serving paper search - 2026-04-27 evening v4 (strict filtering + broader S2)"""
...
# STRICT filter - must clearly be about LLM inference/serving
def is_strictly_llm_serving(title, abstract):
    text = (title + ' ' + (abstract or '')).lower()
    
    # Must contain LLM-related term
    llm_terms = ['llm', 'large language model', 'language model serving',
                 ...
                 'gpt inference', 'llm system', 'generative inference']
    has_llm = any(k in text for k in llm_terms)
    if not has_llm:
        return False
    
    # Must contain serving/inference-specific term
    serving_terms = ['serving system', 'inference system', 'inference engine',
    ...
    # Must have at least 2 serving terms OR 1 very specific serving term
    ss = sum(1 for k in serving_terms if k in text)
    if ss >= 2:
        return True
    if ss >= 1:
        # Check it's not primarily about training/other
        not_serving = ['training', 'fine-tuning', 'pre-training', 'gradient',
                       ...
                       'education', 'hackathon', 'atomistic', 'simulation']
...
# Very specific ArXiv queries
arxiv_queries = [
    "ti:\"llm serving\" OR ti:\"llm inference\"",
    "ti:\"speculative decoding\" AND (ti:llm OR ti:\"large language model\")",
    "ti:\"kv cache\" AND (ti:llm OR ti:\"language model\")",
    "ti:\"inference\" AND ti:\"serving\" AND (ti:llm OR ti:\"large language model\")",
    "ti:\"efficient inference\" AND (ti:llm OR ti:\"transformer\")",
    "ti:\"inference system\" AND (ti:llm OR ti:\"language model\")",
    "ti:\"continuous batching\" OR ti:\"paged attention\"",
    "ti:\"disaggregation\" AND ti:\"inference\"",
    "ti:\"draft\" AND ti:\"speculative\" AND ti:\"llm\"",
]

print("\n🔍 ArXiv Search (strict title matching)")
for q in arxiv_queries:
    ...
    for p in results:
        ...
        if k not in seen and is_strictly_llm_serving(p['title'], p['abstract_en']):
            ...
            all_papers.append(p)
            found += 1
    print(f"    → {found} relevant (strict)")
    time.sleep(5)

# S2 with longer delays
print("\n🔍 Semantic Scholar (with delays)")
s2_queries = [
    "LLM serving inference system optimization",
    "speculative decoding LLM verification draft",
    "KV cache management LLM serving",
    "LLM inference throughput batching scheduling",
...
for i, q in enumerate(s2_queries):
    ...
    if i < len(s2_queries) - 1:
        time.sleep(15)  # longer delays to avoid 429
