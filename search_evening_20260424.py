#!/usr/bin/env python3
"""Evening LLM serving paper search - 2026-04-24"""
...
# ── ArXiv search ──
def search_arxiv(query, max_results=40):
...
arxiv_queries = [
    ...
    "llm inference system scheduling",
...
for q in arxiv_queries:
    ...
    results = search_arxiv(q, max_results=40)
    for p in results:
...
s2_queries = [
    "LLM inference serving system 2025 2026",
    "speculative decoding LLM acceleration",
    "KV cache management LLM inference",
    "LLM serving scheduling batching throughput",
    "LLM inference disaggregation prefill decode",
    "LLM inference kernel flash attention",
    "efficient LLM inference quantization pruning",
    "MoE LLM inference serving system",
    "LLM inference offloading heterogeneous",
...
gh_queries = [
    ...
    "speculative decoding",
    "llm inference engine framework",
    "kv cache llm",
...
for p in all_papers:
    ...
    if tkey in existing_titles or arxiv_id in existing_arxiv:
        continue
    # Validate arxiv ID
    if arxiv_id:
...
# Deduplicate repos and check against DB
new_repos = []
seen_repos = set()
for r in all_repos:
    full_name = r.get('full_name', '')
...
for p in new_papers:
    ...
    for r in new_repos + all_repos:
        ...
        # Match by keywords in title vs repo description
        title_words = [w for w in title_lower.split()[:4] if len(w) > 3]
        if any(w in desc for w in title_words) and r.get('stars', 0) >= 100:
...
gh_added = 0
for r in sorted(new_repos, key=lambda x: x.get('stars', 0), reverse=True)[:15]:
